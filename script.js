...
cats.forEach((c,i)=>{let b=document.createElement('button');b.className='chip'+(i===0?' active':'');b.textContent=c;b.onclick=()=>{document.querySelectorAll('.chip').forEach(x=>x.classList.remove('active'));b.classList.add('active');render(c==='All styles'?'':c)};chips.appendChild(b)});
...
function render(filter=''){const list=currentList(filter);document.querySelector('#products').innerHTML=list.map((p,i)=>`<article class="product"><img class="product-img" src="${raw+p[6]||'deep-wave.jpg'}" onerror="this.src=raw+'deep-wave.jpg'"><span class="badge">${p[5]}</span><small>${p[5].toUpperCase()}</small><h3>${p[1]}</h3><div class="meta">${p[1]}<br>● ${p[3]} · ${p[4]}</div><div class="price">$${p[2]}</div><select><option>${p[3]}</option></select><select><option>${p[4]}</option></select><button class="add" onclick="addCart(${i},'${filter}')">ADD TO CART</button></article>`).join('')}
